                ...
                <div class="panel panel-default">
                    <div class="panel-heading" style="height: 46px;">
                        <h3 class="panel-title">
                            <strong class="pull-left" style="margin-top:6px;">{{ $basket->name }}供应追加物料</strong>
                    ...
                    <div class="panel-body">
                        <div>
                                ...
                                <div class="col-sm-6">
                                    <select class="form-control select2 goods_id" name="goods_id">
                                        ...
                                        @foreach ($goods_records as $goods)
                                            <option @if(old('goods_id') == $goods->id) selected @endif data-unit="{{ $goods->unit }}" value="{{ $goods->id }}">{{ $goods->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group form-group-sm">
                                ...
                                <div class="col-sm-6">
                                    <textarea name="quality_standard" readonly id="quality_standard" class="form-control" style="height: 80px">{{ old('quality_standard') }}</textarea>
                                </div>
                            </div>
                            <div class="form-group
                                ...
                                <div class="col-sm-6">
                                    <div class="radio">
                                        <label>
                                            <input type="radio" @if(old('quantity_type') == 'limited') checked @endif name="quantity_type" value="limited">
                                            可供数量 <input type="text" @if(old('quantity_type') == 'limited') value="{{ old('quantity') }}" @endif name="quantity" style="width: 90px" /> 吨
                                        </label>
                                    </div>
                                    <div class="radio">
                                        <label>
                                            <input type="radio" @if(old('quantity_type', 'infinite') == 'infinite') checked @endif name="quantity_type" value="infinite">
                                            不限数量
                                        </label>
                                    </div>
                                    {{--<div class="input-group input-group-sm">--}}
                                        {{--<input type="text" class="form-control" name="quantity" value="{{ old('quantity') }}" placeholder="可供数量">--}}
                                        {{--<span class="input-group-addon unit">吨</span>--}}
                                    {{--</div>--}}
                                </div>
                                @if ($errors->has('quantity') or $errors->has('quantity_type'))
                                    <p class="col-sm-4 control-label form-msg">{{ $errors->first('quantity') }} {{ $errors->first('quantity_type') }}</p>
                                @endif
                            </div>
                            {{--<div class="form-group--}}
                                {{--@if ($errors->has('price'))--}}
                                    {{--has-error--}}
                                {{--@endif--}}
                                    {{--form-group-sm" style="border-top: 1px solid #dddddd; padding-top: 20px">--}}
                                {{--<label class="col-sm-2 control-label">单价</label>--}}
                                {{--<div class="col-sm-6">--}}
                                    {{--<div class="input-group input-group-sm">--}}
                                        {{--<input type="text" class="form-control" name="price" value="{{ old('price') }}" placeholder="单价">--}}
                            ...
                            {{--<div class="form-group--}}
                                ...
                                {{--<div class="col-sm-6">--}}
                                    {{--<select class="form-control addr select2" multiple name="addresses[]">--}}
                                        {{--<!--<option value="">请选择收货单位</option>-->--}}
                                        {{--@foreach ($companies as $company)--}}
                                            {{--<option @if(in_array($company->name, old('addresses', []))) selected @endif value="{{ $company->name }}">{{ $company->name }}</option>--}}
                                        {{--@endforeach--}}
                                        {{--@foreach ($addresses as $addr)--}}
                                            {{--<option @if(in_array($addr, old('addresses', []))) selected @endif value="{{ $addr }}">{{ $addr }}</option>--}}
                                        {{--@endforeach--}}
                                    {{--</select>--}}
                                    {{--<input type="text" class="form-control" name="custom_address" value="{{ old('custom_address') }}" placeholder="填写其他收货地址">--}}
                                {{--</div>--}}
                                {{--@if ($errors->has('companies'))--}}
                    ...
                    <div class="panel-body">
                        <div>
                            <div class="form-group
                                ...
                                @endif
                                    form-group-sm">
                                <label class="col-sm-2 control-label">款期</label>
                                <div class="col-sm-6" style="margin-top:-5px;">
                                    <div class="radio-inline">
                                        <label>
                                            <input type="radio" @if(old('payment', '先款后货') == '先款后货') checked @endif name="payment" value="先款后货">
                                            先款后货
                                        </label>
                                    </div>
                                    <div class="radio-inline">
                                        <label>
                                            <input type="radio" @if(old('payment') == '货到x天后付款') checked @endif name="payment" value="货到x天后付款">
                            ...
                            <div class="form-group
                                ...
                                <div class="col-sm-6">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" class="form-control" name="price_validity" value="{{ old('price_validity') }}" placeholder="小时数">
                            ...
                            <div class="form-group
                                ...
                                <div class="col-sm-6">
                                    <input type="text" class="form-control date" name="delivery_start" value="{{ old('delivery_start') }}" placeholder="交货日（起）">
                                </div>
                                @if ($errors->has('delivery_start'))
                            ...
                            <div class="form-group
                                ...
                                <div class="col-sm-6">
                                    <input type="text" class="form-control date" name="delivery_stop" value="{{ old('delivery_stop') }}" placeholder="交货日（止）">
                                </div>
                                @if ($errors->has('delivery_stop'))
                            ...
                            <div class="form-group
                                ...
                                <div class="col-sm-6">
                                    <label class="radio-inline">
                                        <input type="radio" name="bargaining" @if(old('bargaining', 'true') == 'true') checked @endif value="true"> 接受
                                    </label>
                                    <label class="radio-inline">
                                        <input type="radio" name="bargaining" @if(old('bargaining')=='false') checked @endif value="false"> 不接受
                                    </label>
                                </div>
                                @if ($errors->has('bargaining'))
            ...
            @if (!empty(old('prices')))
                @foreach (old('prices') as $k => $v)
                    <tr>
                        ...
                        <td>
                            @if (!empty(old('addresses.'.$k)))
                                @foreach (old('addresses.'.$k) as $type => $val_list)
                                    @if (!empty($val_list))
