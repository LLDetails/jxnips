...
@section('main')
    <table style="margin-top: 10px;" class="list table table-striped table-bordered table-hover">
        <tr>
            <td style="color: blue;" colspan="4">{{ $date_start }} - {{ $date_stop }}, 总成交次数 {{ strval((float)$total_count) }}</td>
        </tr>
        <tr>
            <th>#</th>
            <th>供应商</th>
        ...
        @foreach ($data as $k=>$item)
            <tr>
                <td>{{ $k+1 }}</td>
                <td>{{ $item->name }}</td>
                <td>{{ strval((float)$item->count) }}</td>
                <td>{{ sprintf('%.2f', ($item->count/$total_count)*100) }} %</td>
            </tr>
        @endforeach
    </table>
@stop
